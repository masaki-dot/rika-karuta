...
let hostPlayerId = null; // サーバーから知らされるホストのID
let groupId = "";
let gameMode = 'multi';
        ...
        <div style="display:flex; flex-wrap: wrap; gap: 20px;">
            <div style="flex:2; min-width: 300px;">
                ...
                <ol id="end-screen-ranking" style="font-size: 1.2em;">
                    ${ranking.map(p => `<li>${escapeHTML(p.name)}（スコア: ${p.finalScore}｜累計: ${p.totalScore ?? 0}）</li>`).join("")}
                </ol>
    ...
    container.innerHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px;">
            <div id="single-ranking" style="flex: 1; min-width: 300px;">
                <h3>全体ランキング トップ10</h3>
                <ol>
                    ${globalRanking.map((r, i) => `<li style="${r.isMe ? 'font-weight:bold; color:var(--primary-color);' : ''}">${i + 1}. ${escapeHTML(r.name)} - ${r.score}点</li>`).join('')}
                </ol>
            </div>
        </div>
        <hr/>
...
function fixName() {
    ...
    getContainer().innerHTML = `<p>${escapeHTML(groupId)}で待機中...</p>`;
}
...
function updateGameUI(state) {
    ...
    state.current?.cards.forEach(card => {
        ...
        if (card.correct) {
            ...
            chosenByHtml = `<div style="font-size:0.8em; color: black;">${escapeHTML(card.chosenBy)}</div>`;
        } else if (card.incorrect) {
            div.style.background = "crimson";
            div.style.color = "white";
            chosenByHtml = `<div style="font-size:0.8em;">${escapeHTML(card.chosenBy)}</div>`;
        } else if (card.correctAnswer) {
            div.style.background = "lightgreen";
            div.style.border = "2px solid green";
        }

        div.innerHTML = `<div style="font-weight:bold; font-size:1.1em;">${escapeHTML(card.term)}</div>${chosenByHtml}`;
        div.onclick = () => {
    ...
    if(myPlayer) {
        myInfoDiv.innerHTML = `<h4>自分: ${escapeHTML(myPlayer.name)} (正解: ${myPlayer.correctCount ?? 0})</h4>${renderHpBar(myPlayer.hp)}`;
    }
    const othersInfoDiv = document.getElementById('others-info');
    othersInfoDiv.innerHTML = '<h4>他のプレイヤー</h4>';
    otherPlayers.forEach(p => {
        othersInfoDiv.innerHTML += `<div><strong>${escapeHTML(p.name)} (正解: ${p.correctCount ?? 0})</strong>${renderHpBar(p.hp)}</div>`;
    });
}
...
function updateSinglePlayGameUI(state) {
    ...
    document.getElementById('single-player-info').innerHTML = `<h4>スコア: ${state.score}</h4>`;
}
...
socket.on("assigned_group", (newGroupId) => {
    groupId = newGroupId;
    getContainer().innerHTML = `<h2>あなたは <strong>${escapeHTML(groupId)}</strong> に割り振られました</h2><p>ホストが開始するまでお待ちください。</p>`;
});
...
socket.on("host_state", (allGroups) => {
    ...
    div.innerHTML = '<h3>各グループの状況</h3>' + Object.entries(allGroups).map(([gId, data]) => {
        ...
        const pauseButton = `<button class="pause-btn" data-groupid="${gId}">${data.isPaused ? '再開' : '一時停止'}</button>`;
    ...
    document.querySelectorAll('.pause-btn').forEach(button => {
        button.onclick = (e) => {
            const groupId = e.target.dataset.groupid;
            socket.emit('host_toggle_pause', { groupId });
        };
    });
});
